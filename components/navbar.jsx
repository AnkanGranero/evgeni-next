  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 h-[9rem] md:h-[8rem] pb-[1rem] md:p-0 bg-orange content-grid">
      ...
      >
        ...
        <h1 className="text-matte-black whitespace-nowrap tracking-[-0.07em] flex font-bold text-[3.5rem] md:text-7xl  xl:text-8xl leading-[1.5] w-full md:w-1/2 lg:w-1/3 justify-center md:p-[0] font-evgeni">
          Evgeni Leonov
        </h1>

        <ul className="bg-orange w-full fixed bottom-0 lg:relative p-3 lg:p-0 lg:pt-5 flex lg:w-1/4 xl:w-1/3 justify-evenly">
          {views.map((view) => (
            <li key={view.name}>
              <Link href={view.link} className="text-2xl lg:text-3xl font-medium text-matte-black font-evgeni">
                {view.name}
              </Link>
            </li>
          ))}
